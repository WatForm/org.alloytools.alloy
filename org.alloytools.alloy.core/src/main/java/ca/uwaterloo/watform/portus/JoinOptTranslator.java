...
final class JoinOptTranslator extends AbstractTranslator {
    ...
    public Term translate(TermTuple tuple, ExprBinary expr, TranslationContext context) {
        ...
        Pair<AnnotatedTerm, AnnotatedTerm> leftScalar = scalarCaster.castToScalar(expr.left, context);
        if (leftScalar != null) {
            AnnotatedTerm leftTerm = leftScalar.a;
            Term leftGuard = leftScalar.b.getTerm();
        ...
        Pair<AnnotatedTerm, AnnotatedTerm> rightScalar = scalarCaster.castToScalar(expr.right, context);
        if (rightScalar != null) {
            AnnotatedTerm rightTerm = rightScalar.a;
            Term rightGuard = rightScalar.b.getTerm();
