...
//        // We translate the domain constraint for "sig A { f: M e }" as follows:
//        // [[all this: A | this.f in this.(A->M e)]] -> DOESN'T WORK LOL
//        // The name "this" has to be used so it matches with any occurrences of "this" in e generated by Alloy.
//        // With the join optimization enabled, this should generate the same MSFOL as [[f in A->M e]], while also
//        // elegantly handling "this" (used when the bounding expression refers to another field) and multiplicity
//        // constraints (handled by the declaration formula translation routine when recursing to A->M e).
//        Expr domainExpr = getFieldDomainExpr(field);
//        Decl thisDecl = field.sig.oneOf("this");
//        Expr thisVar = thisDecl.get();
//
//        Expr axiom = thisVar.join(field).in(thisVar.join(domainExpr)).forAll(thisDecl);
//        return recursivelyTranslate(axiom, context);

//        // If there's a "this" in the bound, it gets complicated.
//        // Alloy generates a variable with label "this" when the bound refers to a previous field in the sig:
//        // e.g. for "sig A {f: e; g: f}", g will have bound "this.f"
//        Expr fieldBoundWithMult = field.decl().expr.deNOP();
//        Expr fieldBound = stripMultiplicity(fieldBoundWithMult);
//        Decl thisDecl = field.sig.oneOf("value");
//        Expr thisVar = thisDecl.get();
//        Pair<Boolean, Expr> thisSubResult = PortusUtil.substituteThis(fieldBound, thisVar);
//        boolean hasThis = thisSubResult.a;
//
//        if (hasThis) {
//            // There was a "this": it gets messy.
//            // We substituted "this" with thisVar, so we have an axiom "all thisVar: sig | thisVar.field in bound".
//            // We also manually add an axiom to deal with the multiplicity: if "sig A {f: e, g: M f}" where M is the
//            // multiplicity, we also have to assert "M thisVar.field" for all thisVar.
//            Expr subbedBound = thisSubResult.b;
//            Expr constraint = thisVar.join(field).in(subbedBound);
//
//            switch (fieldBoundWithMult.mult()) {
//                case ONEOF:
//                    constraint = constraint.and(thisVar.join(field).one());
//                    break;
//                case LONEOF:
//                    constraint = constraint.and(thisVar.join(field).lone());
//                    break;
//                case SOMEOF:
//                    constraint = constraint.and(thisVar.join(field).some());
//                    break;
//            }
//
//            return recursivelyTranslate(constraint.forAll(thisDecl), context);
//        } else {
//            // See KT 4.2 (page 29).
//            // If there's no "this", for "sig A {f: M e}", just use "f in A->M e" as the axiom.
//            // This simplifies dealing with multiplicities on the arrow: the declaration formula translation handles it.
//            Expr domainExpr = getFieldDomainExpr(field);
...
//        declared = stripMultiplicity(declared);
//        switch (mult) {
//            case ONEOF:
